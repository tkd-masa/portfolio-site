<template>
  <LoaderArea />
    <HeaderArea />
    <HeroimgArea @stopScroll="stopScroll" @openScroll="openScroll" />
    <main>
      <div class="inner">
        <AboutArea />
        <WorksArea />
...
export default {
  ...
  data(){
    return {
      ...
      openScroll:this.$openScroll()
    }
  },
  components: {
    LoaderArea,HeaderArea,HeroimgArea,AboutArea,WorksArea,SkillsArea,FooterArea
  },
  mounted() { 
...
<style>
  ...
  * {
  ...
  /* スクロールのアニメーション設定 */
  .scroll_ani {
      transition: all 1s cubic-bezier(.33,.56,.56,1);
